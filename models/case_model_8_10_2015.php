    ...
    function __construct()
    {
		...
		//error_reporting(0);
	       ...
	       function insertActivationcode($userEmail,$activation_id)
			...
			public function getfileans($caseid){
			   $query = $this->db->query("SELECT caseanswer.anstxt, file_upload.f_name from caseanswer LEFT JOIN file_upload ON caseanswer.anstxt = file_upload.refrenceid WHERE caseanswer.caseid = '$caseid' ORDER BY caseanswer.IDQuest ASC");
			   if($query->num_rows() > 0){
			   return $query->result();
			   }else{
				  return 0;
...
function uploadimg($userid,$caseid,$qid,$file){
    ...
    $targetPath = getcwd(). '/uploads/';  
	 ...
	 if($file['name'] !== ''){
    $file_name=time().'-'.$qid.'-'. $file['name'];
	$refrenceid =  $userid.$caseid.$qid.uniqid();
	$fname =  $targetPath.$userid.$caseid.$qid.$file_name;  
	$ftype=$file["type"];
