...
class Case_management extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('assigned_manager_case',$data);
	}
	    ...
	    /* public function delete_vec()
		  ...
		  public function add_edit_user($id='')
		 {
				...
				$user_info['Password']                = strip_tags(->input->post('password'));
			  ...
			  if(isset($_REQUEST['btn_update']))
				{
					...
					elseif($this->input->post('ManagerAssigned')==0 && $this->input->post('EndDate')==0)
					{
				  	  $data['CaseStatus']                  = '1';
					  $data['ManagerAssigned']             = strip_tags($this->input->post('ManagerAssigned'));
					}
